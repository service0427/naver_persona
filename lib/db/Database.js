...
class Database {
  ...
  /**
   * 프로필 데이터 저장 (쿠키, 히스토리, Preferences + VPN IP)
   * @param {Object} data - { threadId, chromeVersion, vpnIp, fingerprint, cookies, history, preferences, result }
   */
  async saveProfileData(data) {
    ...
    try {
      await this.pool.execute(`
        ...
        (profile_key, thread_id, chrome_version, vpn_ip, fingerprint, cookies, history, preferences, total_uses, last_result)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?)
        ON DUPLICATE KEY UPDATE
