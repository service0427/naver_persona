...
class Database {
  ...
  async initTables() {
    ...
    try {
      ...
      // 프로필 데이터 테이블 (프로필별 쿠키/히스토리/localStorage 저장)
      await conn.execute(`
  ...
  /**
   * 프로필 데이터 저장 (쿠키, localStorage, 히스토리, Preferences + VPN IP)
   * @param {Object} data - { threadId, chromeVersion, vpnIp, fingerprint, cookies, origins, history, preferences, result }
   */
  async saveProfileData(data) {
    ...
    try {
      await this.pool.execute(`
        ...
        (profile_key, thread_id, chrome_version, vpn_ip, fingerprint, cookies, origins, history, preferences, total_uses, last_result)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)
        ON DUPLICATE KEY UPDATE
